...
int main(){
    ...
    fx[0] = x[0]*x[0]+x[0]+1; //f(x[0])を計算

    grad[0] = 2*x[0]+1; //勾配を計算

    for (int i=0; i<n; i++){
        if(abs(grad[i])>eps){
            x[i+1] = x[i] - a * grad[i]; //xを更新
            fx[i+1] = x[i+1]*x[i+1]+x[i+1]+1; //f(x[i+1])を計算
            grad[i+1] = 2*x[i+1]+1; //勾配を更新
        }else{
            cnt = i+1; //繰り返し回数を記憶
            break; // 勾配がeps以下なら終了
        }
    ...
    // ファイルに出力
    ofstream outFile("report-4-1-AJG23005.txt");
    if (outFile.is_open()) {
        outFile << "f(x)=x*x+x+1" << endl;
        for (int i=0; i<cnt; i++){
